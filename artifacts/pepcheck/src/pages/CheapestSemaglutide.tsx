...
export default function CheapestSemaglutide() {
  ...
  const { data: priceRange } = useGetMedicationPriceRange({ medication: "semaglutide" });
  const trackClick = useTrackProviderClick();

  const handleVisit = (providerId: number, website?: string | null) => {
    trackClick.mutate({ data: { providerId, source: "cheapest-semaglutide" } });
    ...
    <div className="w-full">
      ...
      <section className="bg-primary text-primary-foreground py-16 md:py-24 px-4">
        <div className="container mx-auto max-w-5xl">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4 max-w-3xl">
            Cheapest Compounded Semaglutide Providers
          </h1>
          <p className="text-primary-foreground/80 text-lg max-w-2xl mb-8">
            Transparent pricing for compounded semaglutide (generic Ozempic/Wegovy) across every vetted telehealth provider. Sorted lowest price first.
          </p>
      ...
      <section className="bg-blue-50 border-b border-blue-200 px-4 py-3">
        <div className="container mx-auto max-w-5xl flex items-center gap-2 text-sm text-blue-800">
          ...
          <p>Prices are per vial (self-pay). Vial concentrations and sizes vary by provider — always compare mg/mL and total mg when evaluating costs.</p>
        </div>
      </section>
      ...
      <section className="py-12 px-4">
        <div className="container mx-auto max-w-5xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h2 className="text-2xl font-bold">Semaglutide Pricing Table</h2>
              <p className="text-muted-foreground text-sm mt-1">{providers?.length || 0} providers · sorted lowest price first</p>
            </div>
            <select
              value={stateFilter}
                    ...
                    <tr key={provider.id} className={`border-b hover:bg-muted/20 transition-colors ${i === 0 ? "bg-green-50/50" : ""}`} data-testid={`row-provider-${provider.id}`}>
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          {i === 0 && (
                            <span className="bg-green-100 text-green-800 text-xs font-bold px-2 py-0.5 rounded shrink-0">LOWEST</span>
                          )}
          ...
          <div className="mt-4 text-center">
            <Button variant="outline" asChild>
              <Link href="/compare?medication=semaglutide"><DollarSign className="w-4 h-4 mr-2" /> Advanced Filter & Compare</Link>
            </Button>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-muted/30 border-t px-4">
        <div className="container mx-auto max-w-3xl space-y-4 text-sm text-muted-foreground leading-relaxed">
          <h2 className="text-2xl font-bold text-foreground">About Compounded Semaglutide Pricing</h2>
          <p>
            Compounded semaglutide is the generic equivalent of Ozempic (diabetes) and Wegovy (weight loss). It became widely accessible as an alternative to brand-name GLP-1 medications when FDA shortages created demand that brand manufacturers couldn't meet.
          </p>
          <p>
            Typical prices for compounded semaglutide range from $199 to $450 per vial, compared to $800–$1,300 per month for brand-name Wegovy or Ozempic without insurance. The savings are significant — but quality and sourcing matter. All providers on Pepcheck are verified to source from licensed compounding pharmacies.
          </p>
          <p>
            <strong className="text-foreground">Note on FDA status:</strong> The FDA shortage designation for semaglutide has been contested. Check with your provider for the current regulatory status before ordering.
          </p>
          <div className="flex gap-3 pt-2">
            <Button asChild><Link href="/compare?medication=semaglutide&sort=price_asc">Compare All Options</Link></Button>
